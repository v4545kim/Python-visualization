...
class Visualization:
    ...
    def makeWordCloud(self):
        Sun_color_file = './../Image/Sun.png'
        Sun_coloring = np.array(Image.open(Sun_color_file))

        fontpath = 'AppleGothic'
        wordcloud = WordCloud(font_path=fontpath, mask=Sun_coloring,
                              relative_scaling=0.2, background_color='lightyellow')

        wordcloud = wordcloud.generate_from_frequencies(self.wordDict)

        image_colors = ImageColorGenerator(Sun_coloring)
        ...
        filename = '2019weatherInfoWordCloud.png'
    ...
    def makeBarChart(self):
        ...
        filename = '2019weatherInfoBarChart.png'
        plt.savefig(filename, dpi=400)
        print(filename + '파일 저장됨')
