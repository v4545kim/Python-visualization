...
class Visualization:
    ...
    def makeWordCloud(self):
        Umbrella_color_file = './../Image/Umbrella.png'
        Umbrella_coloring = np.array(Image.open(Umbrella_color_file))

        fontpath = 'AppleGothic'
        wordcloud = WordCloud(font_path=fontpath, mask=Umbrella_coloring,
                              relative_scaling=0.2, background_color='white')

        wordcloud = wordcloud.generate_from_frequencies(self.wordDict)

        image_colors = ImageColorGenerator(Umbrella_coloring)
        ...
        filename = '2021weatherInfoWordCloud.png'
    ...
    def makeBarChart(self):
        ...
        filename = '2021weatherInfoBarChart.png'
        plt.savefig(filename, dpi=400)
        print(filename + '파일 저장됨')
